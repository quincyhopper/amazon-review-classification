...
│   └── 200.glove.txt       # GloVe embeddings - download manually! (ignored by git)
...
## Backpropagation
The `FeedForwardNetwork` class in `nn.py` implements a manual backpropagation engine for an arbitrary number of layers, each containing an arbitrary number of neurons/units. The class uses the chain rule to compute the gradient of the output with respect to every layer in the network (Linear or ReLU). This allows you to test a variety of architectures without having to rewrite the logic. **Note that ReLU activation is hard-coded**.

## AdamW optimisation
The `Adam` class in `optim.py` allows the model to update its weights and biases using the AdamW optimisation algorithm.

**Key features**
- **Adaptive learning rates**: The optimiser computes an estimate for the first moment (expected value) and second moment (variance) of each parameter. When we update the parameters using these moments, we are using a moving average that lends increasingly less weight to old gradients.
- **Bias correction**: Since we initialise the moments to zero, the first few moment estimates are biased towards zero, which would make the gradients vanish. To correct for this, we computed a bias-corrected learning rate for each time step, which increases the moment estimates during early training.
- **Decoupled weight decay**: Instead of applying weight decay to weights' gradients, AdamW applies it directly to the weights before any updates occur. This has been shown to produce better generalisations.
...
where $N$ is the number of reviews in the dataset and $\text{df}_i$ is the frequency of token $i$ across the entire dataset. If the token is extremely frequent (and therefore potentially unimportant), the IDF score is very low.
